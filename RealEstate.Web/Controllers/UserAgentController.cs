...
namespace RealEstate.Web.Controllers
{
    ...
    public class UserAgentController : Controller
    {
        ...
        public UserAgentController(ApplicationUserManager userManager, ApplicationRoleManager roleManager)
        {
        ...
        public ActionResult List(int PageNo = 1, int PageSize = 10, string searchterm = "")
        {
            ...
            objUsers.AgentClientView.ClientList = objUser.GetAgentClientList(PageNo, PageSize, out TotalRow, AgentID.Id, searchterm);
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.ActiveURL = "/UserAgent/List";
        ...
        public async Task<ActionResult> ResendEmail(string Id)
        {
            ...
            if (result.Succeeded)
            {
                int MailStatus = SendEmailConfirmationToken(Id, user.Name, "Forgot Password From Just BE", user.Email, TempPassword);
                if (MailStatus == 0)
                {
                    TempData["MailMsg"] = 1;
                    return RedirectToAction("list", "useragent");
                }
                else
                {
                    TempData["MailMsg"] = 0;
        ...
        private int SendEmailConfirmationToken(string userID, string Name, string subject, string email, string pass)
        {
            ...
            mm.Body = this.PopulateBody(Name, email, pass);
            mm.IsBodyHtml = true;
        ...
        public ActionResult autocomplete(string term)
        {
            var AgentID = UserManager.FindByEmail(User.Identity.Name);
            UserListViewModel objUsers = new UserListViewModel();
            objUsers.ListSearchAutoComplete = objUser.UserAutoComplete(term, AgentID.Id);
