...
$(function () {
    ...
    $(".content").on("click", "#txtAIPreSet", function () {
        $(this).datepicker({
            changeMonth: true,
            changeYear: true,
            dateFormat: "mm/dd/yy",
            onSelect: function (date, inst) {
                $('.chkStep').each(function () {
                    ...
                    if (statusId == 16) {
                        var D1, D2;
                        var StartDate = $('#txtAIPreSet').parent().prev('div').find('.txtDate').val()
                        var EndDate = date;
                        if (location.hostname === "localhost" || location.hostname === "127.0.0.1" || location.hostname === "") {


                            var temp = StartDate.split('-');
                            var D1 = temp[1] + "-" + temp[0] + "-" + temp[2];


                            var temp1 = EndDate.split('-');
                            var D2 = temp1[1] + "-" + temp1[0] + "-" + temp1[2];
                            ...
                            D1 = new Date(StartDate.toString())
                            D2 = new Date(EndDate.toString())
                        }
                        var days = getDays(D2, D1);

                        $(this).parent().parent().next('div').find('.getDate').val(days);
                        //  AccessRules(date, statusId, '', days);
                    }
    ...
    $(".content").on("change", ".chkStep", function (e) {
        ...
        if ($(this).is(':checked')) {
            ...
            if ($(this).parent().find('input').attr("name") == "[5].IsCompleted") {
                if ($("#spnAddress").text() != '') {
    ...
    //$(".content").on("change", "#drpCondoType", function () {
    //    var getDay = $("#chkCondo").parent().parent().next('div').find('.getDate')
    //    var Area = $('#MstClosingConfig_utblMstClosingDate_StateId option:selected').text();
    //    var HomeType = $('#MstClosingConfig_utblMstClosingDate_HomeType option:selected').text();
    //    if ($(this).val() == "") {
    //        AccessRules('', 18, 'No', getDay, Area, HomeType);
    //    }
    //    else {
    //        AccessRules('', 18, 'Yes', getDay, Area, HomeType);
    //    }
...
function loadData() {
    $.ajax({
        url: "/Coordinator/DealTrackerV2/DealDocumentList",
        type: "GET",
        data: {
        ...
        success: function (result) {
            ...
            if (Object.keys(result.MstBuyerDocumentListView).length > 0) {
                ...
                html += '                      </div>'
                $.each(result.MstBuyerDocumentListView, function (key, item) {
                    ...
                    html += '<div class="tableCell"><a href="/UploadedFiles/TrackDeal/' + item.DealTrackDocID + item.TrackDocFile + '" class="download-link" target="_blank" ><img src="/Content/NewVersion/Admin/img/download-icon.svg" /></a></div>'
                    html += '<div class="tableCell"><a href="#!" data-TrackingId="' + item.TrackingID + '"  data-DocId="' + item.DealTrackDocID + '"  class="delete-link" onclick="DeleleDocument(this)"><img src="/Content/NewVersion/Admin/img/delete-icon.svg" /></a></div>'
                    html += '</div>'
                });
            }
            else {
...
function AddDocument() {
    ...
    $.ajax({
        url: "/Coordinator/DealTrackerV2/AddDealDocument",
        data: dataString,
        type: "POST",
        cache: false,
        contentType: false,
...
function DeleleDocument(obj) {
    ...
    if (ans) {
        ...
        $.ajax({
            url: "/Coordinator/DealTrackerV2/DeleteDealDocument?DealTrackDocID=" + ID + "&TrackingID=" + TracId,
            type: "POST",
            contentType: "application/json;charset=UTF-8",
            dataType: "json",
            success: function (result) {
...
$('#txtDocTitle').autocomplete({
    source: function (request, response) {
        ...
        $.ajax({
            type: 'POST',
            url: '/Coordinator/DealTrackerV2/Documentautocomplete/',
            dataType: 'json',
